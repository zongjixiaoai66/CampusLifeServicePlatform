<template>
  <div class="main-content">
    ...
    <div v-if="showFlag">
      <el-form :inline="true" :model="searchForm" class="form-content">
        <el-row :gutter="20" class="slt"
          ...
          <el-form-item :label="contents.inputTitle == 1 ? '活动名称' : ''">
            <el-input prefix-icon="el-icon-search" v-model="searchForm.wenyuName" placeholder="活动名称"
                      clearable></el-input>
          </el-form-item>
        ...
        <el-row class="ad"
          ...
          <el-form-item>
            ...
            <a style="text-decoration:none" class="el-button el-button--success"
               v-if="isAuth('wenyu','导入导出')"
               icon="el-icon-download"
               href="http://localhost:8080/xyshfwpt/upload/wenyuMuBan.xls"
            >批量导入文娱活动数据模板</a>
            &nbsp;
            <el-upload
                v-if="isAuth('wenyu','导入导出')"
                ...
                :action="getActionUrl"
                :on-success="wenyuUploadSuccess"
                :on-error="wenyuUploadError"
            ...
            &nbsp;
            <!-- 导出excel -->
            <download-excel v-if="isAuth('wenyu','导入导出')" style="display: inline-block" class="export-excel-wrapper"
                            :data="dataList" :fields="json_fields" name="wenyu.xls">
              <!-- 导出excel -->
              <el-button
                  type="success"
                  icon="el-icon-download"
      ...
      <div class="table-content">
        <el-table class="tables" :size="contents.tableSize" :show-header="contents.tableShowHeader"
          ...
          <el-table-column :sortable="contents.tableSortable" :align="contents.tableAlign"
                           prop="wenyuUuidNumber"
                           header-align="center"
                           label="活动编号">
            <template slot-scope="scope">
              {{ scope.row.wenyuUuidNumber }}
            </template>
          </el-table-column>
          ...
          <el-table-column :sortable="contents.tableSortable" :align="contents.tableAlign"
                           prop="wenyuKucunNumber"
                           header-align="center"
                           label="活动人数">
            <template slot-scope="scope">
              {{ scope.row.wenyuKucunNumber }}
            </template>
          </el-table-column>
          ...
          <el-table-column width="300" :align="contents.tableAlign"
            ...
            <template slot-scope="scope">
              ...
              <el-button v-if="isAuth('wenyu','查看')" type="success" icon="el-icon-tickets" size="mini"
                         @click="addOrUpdateHandler(scope.row.id,'info')">详情
              </el-button>
              <el-button v-if="isAuth('wenyu','修改')" type="primary" icon="el-icon-edit" size="mini"
                         @click="addOrUpdateHandler(scope.row.id)">修改
              </el-button>
              <el-button v-if="isAuth('wenyu','删除')" type="danger" icon="el-icon-delete" size="mini"
                         @click="deleteHandler(scope.row.id)">删除
              </el-button>
              <!--<el-button v-if="isAuth('wenyu','修改')" type="primary" icon="el-icon-add-location" size="mini" @click="plusWenyuKucunNumber(scope.row.id,scope.row.wenyuKucunNumber)">增加活动人数</el-button>-->
              <!--<el-button v-if="isAuth('wenyu','修改')" type="primary" icon="el-icon-delete-location" size="mini" @click="reduceWenyuKucunNumber(scope.row.id,scope.row.wenyuKucunNumber)">减少活动人数</el-button>-->
    ...
    <!-- 库存 -->
    <el-dialog
        :title="wenyuKucunNumberTitle"
        :visible.sync="wenyuKucunNumberVisible"
        width="30%">
      <span>操作数量</span>
      <el-input-number v-model="kucunNumber" :min="1" :max="100" label="描述文字"></el-input-number>
      <span slot="footer" class="dialog-footer">
			<el-button @click="wenyuKucunNumberVisible = false">取 消</el-button>
			<el-button type="primary" @click="addWenyuKucunNumber()">确 定</el-button>
		  </span>
    </el-dialog>
  ...
  data() {
    return {
      ...
      form: {
        ...
        yonghuId: null,
        wenyuName: null,
        wenyuUuidNumber: null,
        wenyuPhoto: null,
        wenyuAddress: null,
        wenyuTypes: null,
        wenyuKucunNumber: null,
        wenyuContent: null,
        wenyuDelete: null,
        insertTime: null,
        createTime: null,
      ...
      wenyuKucunNumber: 0,
      //标题
      wenyuKucunNumberTitle: null,
      //模态框状态
      wenyuKucunNumberVisible: false,
    };
  },
  created() {
  ...
  methods: {
    chartDialog() {
      ...
      let params = {
        ...
        thisTable: {//当前表
          tableName: 'wenyu',//当前表表名,
          // sumColum: 'wenyu_number', //求和字段
          date: 'insert_time',//分组日期字段
          // string : 'wenyu_name',//分组字符串字段
          types : 'wenyu_types',//分组下拉框字段
        },
        // joinTable : {//级联表（可以不存在）
    ...
    getDataList() {
      ...
      params['wenyuDelete'] = 1// 逻辑删除字段 1 未删除 2 删除
    ...
    // 导入功能上传文件成功后调用导入方法
    wenyuUploadSuccess(data) {
      let _this = this;
      _this.$http({
        url: "wenyu/batchInsert?fileName=" + data.file,
        method: "get"
      }).then(({data}) => {
        if (data && data.code === 0) {
          _this.$message({
    ...
    // 导入功能上传文件失败后调用导入方法
    wenyuUploadError(data) {
      this.$message.error('上传失败');
    },
    //增加活动人数
    plusWenyuKucunNumber(thisId, wenyuKucunNumber) {
      //清空数据
      //活动人数数据设置为0
      this.kucunNumber = 0
      //id清空
      ...
      //将模态框打开
      this.wenyuKucunNumberVisible = true;
      //标题
      this.wenyuKucunNumberTitle = '增加活动人数'
    },
    //减少活动人数
    reduceWenyuKucunNumber(thisId, wenyuKucunNumber) {
      //清空数据
      //活动人数数据设置为0
      this.kucunNumber = 0
      //id清空
      ...
      this.wenyuKucunNumberVisible = true;
      //标题
      this.wenyuKucunNumberTitle = '减少活动人数'
    },
    //提交用户操作活动人数数量
    addWenyuKucunNumber() {
      var type = null
      ...
      if (this.kucunNumberTypes == "+") {
        type = '增加'
        maxNumber = this.wenyuKucunNumber + this.kucunNumber
      } else {
        type = '减少'
        if (this.wenyuKucunNumber < this.kucunNumber || this.kucunNumber <= 0) {
          this.$message.error('活动人数不足不能减少');
          return false;
        } else {
          maxNumber = this.wenyuKucunNumber - this.kucunNumber
        }
      ...
      this.wenyuKucunNumberVisible = false;
    },
    //无用
    wuyong(id) {
      let _this = this;
