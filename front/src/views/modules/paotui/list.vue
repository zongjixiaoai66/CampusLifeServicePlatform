    ...
    <div v-if="showFlag">
        ...
        <el-row class="ad"
          ...
          <el-form-item>
            ...
            <a style="text-decoration:none" class="el-button el-button--success"
               v-if="isAuth('paotui','导入导出')"
               icon="el-icon-download"
               href="http://localhost:8080/xyshfwpt/upload/paotuiMuBan.xls"
            >批量导入跑腿数据模板</a>
            &nbsp;
            <el-upload
                v-if="isAuth('paotui','导入导出')"
                ...
                :action="getActionUrl"
                :on-success="paotuiUploadSuccess"
                :on-error="paotuiUploadError"
            ...
            &nbsp;
            <!-- 导出excel -->
            <download-excel v-if="isAuth('paotui','导入导出')" style="display: inline-block"
                            class="export-excel-wrapper" :data="dataList" :fields="json_fields" name="paotui.xls">
              <!-- 导出excel -->
              <el-button
                  type="success"
                  icon="el-icon-download"
      ...
      <div class="table-content">
        <el-table class="tables" :size="contents.tableSize" :show-header="contents.tableShowHeader"
          ...
          <el-table-column :sortable="contents.tableSortable" :align="contents.tableAlign"
                           prop="paotuiUuidNumber"
                           header-align="center"
                           label="跑腿编号">
            <template slot-scope="scope">
              {{ scope.row.paotuiUuidNumber }}
            </template>
          </el-table-column>
          ...
          <el-table-column width="300" :align="contents.tableAlign"
            ...
            <template slot-scope="scope">
              ...
              <el-button v-if="isAuth('paotui','查看')" type="success" icon="el-icon-tickets" size="mini"
                         @click="addOrUpdateHandler(scope.row.id,'info')">详情
              </el-button>
              <el-button v-if="isAuth('paotui','修改')" type="primary" icon="el-icon-edit" size="mini"
                         @click="addOrUpdateHandler(scope.row.id)">修改
              </el-button>
              <el-button v-if="isAuth('paotui','删除')" type="danger" icon="el-icon-delete" size="mini"
                         @click="deleteHandler(scope.row.id)">删除
              </el-button>
              <el-button v-if="isAuth('paotui','修改')" type="primary" icon="el-icon-tickets" size="mini"
  ...
  methods: {
    chartDialog() {
      ...
      let params = {
        ...
        thisTable: {//当前表
          tableName: 'paotui',//当前表表名,
          // sumColum: 'paotui_number', //求和字段
          date: 'insert_time',//分组日期字段
          // string : 'paotui_name',//分组字符串字段
          types : 'paotui_types',//分组下拉框字段
        },
        // joinTable : {//级联表（可以不存在）
    ...
    getDataList() {
      ...
      params['paotuiDelete'] = 1// 逻辑删除字段 1 未删除 2 删除
    ...
    // 导入功能上传文件成功后调用导入方法
    paotuiUploadSuccess(data) {
      let _this = this;
      _this.$http({
        url: "paotui/batchInsert?fileName=" + data.file,
        method: "get"
      }).then(({data}) => {
        if (data && data.code === 0) {
          _this.$message({
